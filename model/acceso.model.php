...
class AccesoModel {
    ...
    public function RegistrarUsuario($username, $email, $passwordHash) {
        try {
            // Obtiente el mes actual para la columna de reinicio
            $mesActual = date('n');
            ...
            // Ejecuta la inserción con los datos recibidos
            return $stm->execute([$username, $email, $passwordHash, $mesActual]);

        } catch (PDOException $e) {
            // Si hay un error (por ejemplo, la base de datos se cae), lo muestra
    ...
    public function LoguearUsuario($email) {
        try {
            ...
            // Devuelve el objeto con los datos o 'false' si no existe
            return $stm->fetch(PDO::FETCH_OBJ);
    ...
    public function ActualizarUsuario($id, $username, $foto, $passwordHash = null) {
        try {
            ...
            // Si hay un error (por ejemplo, nombre inválido), devolvemos false
            return false;
        }
    }
